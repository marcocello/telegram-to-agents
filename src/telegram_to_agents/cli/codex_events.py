...
def _dispatch_codex_event(data: dict[str, Any]) -> list[StreamEvent]:
    ...
    event_type = data.get("type", "")
    bridge_event = _parse_bridge_control_event(data, event_type)
    ...
    if event_type == "thread.started":
        ...
        return [
            SystemInitEvent(
                ...
                session_backend=(
                    data["transport"] if data.get("transport") in {"managed", "embedded"} else None
                ),
            ),
        ]
    ...
    if event_type == "turn.completed":
        logger.debug("Codex event parsed type=%s", event_type)
        return [ResultEvent(type="result")]
...
def _parse_bridge_control_event(
    data: dict[str, Any],
    event_type: object,
) -> list[StreamEvent] | None:
    if event_type == "system" and data.get("subtype") == "status":
    ...
    if event_type == "progress" and isinstance(data.get("text"), str):
        return [ProgressUpdateEvent(type="assistant", subtype="progress", text=data["text"])]
    return None
