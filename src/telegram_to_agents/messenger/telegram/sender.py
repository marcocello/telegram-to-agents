...
from telegram_to_agents.files.tags import (
    FILE_PATH_RE,
    extract_file_paths,
    guess_mime,
    path_from_file_tag,
)
from telegram_to_agents.messenger.telegram.formatting import (
    markdown_to_telegram_html,
    split_html_message,
)
from telegram_to_agents.security.paths import is_path_safe
...
async def send_rich(
    ...
    network error cut it short (#160). File failures do not affect the return value.
    """
    ...
    if clean_text:
        _last_msg, delivered = await _send_text_chunks(
            bot,
            chat_id,
            clean_text,
