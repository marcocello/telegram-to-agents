"""Download Telegram media and preserve it as a structured user turn."""
...
from telegram_to_agents.cli.types import Attachment, UserTurn
from telegram_to_agents.files.storage import prepare_destination as _prepare_destination
from telegram_to_agents.files.storage import sanitize_filename as _sanitize_filename
...
class AutomaticTranscription:
    """Runtime inputs required by automatic Telegram audio transcription."""
...
@dataclass(frozen=True, slots=True)
class MediaInfo:
    """Metadata for one received Telegram attachment."""
...
async def resolve_media_turn(
    bot: Bot,
    message: Message,
    telegram_files_dir: Path,
    *,
    automatic_transcription: AutomaticTranscription | None = None,
) -> UserTurn | None:
    """Download media and return text plus the unchanged local attachment.
    ...
    try:
        ...
        await bot.send_message(message.chat.id, "Could not download that file.")
        return None
    ...
    if (
        ...
        and automatic_transcription is not None
        and automatic_transcription.config.automatic_audio
    ):
        from telegram_to_agents.transcription import (
            AudioTranscriptionError,
            transcribe_openai_audio,
        )

        try:
            transcript = await transcribe_openai_audio(
                info.path,
                automatic_transcription.config,
                automatic_transcription.env_file,
            )
        except AudioTranscriptionError:
            logger.warning("Telegram audio transcription failed", exc_info=True)
            ...
            return None
        return UserTurn(text=build_audio_transcript_prompt(transcript, info.caption))

    return UserTurn(
        text=info.caption or "",
        attachments=(Attachment(path=info.path.resolve(), media_type=info.media_type),),
    )


def build_audio_transcript_prompt(transcript: str, caption: str | None) -> str:
    """Build the user turn sent to Codex after automatic transcription."""
