...
- Read-back: same-topic turns execute serially and the second resumes the native ID persisted by the first; `/new` causes a new native ID; the provider switch starts a fresh provider-native session and persists its provider instead of sending an incompatible ID.
- Fake: Telegram Bot API and external provider processes only.
