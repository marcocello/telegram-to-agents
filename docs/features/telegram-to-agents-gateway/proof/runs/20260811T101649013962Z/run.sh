...
(
  ...
  PYTHONPATH="$wheel_site" "$python_bin" -c \
    "from pathlib import Path; import ductor_bot; from ductor_bot.cli.claude_provider import ClaudeCLI; from ductor_bot.cli.codex_provider import CodexCLI; from ductor_bot.messenger.telegram.app import TelegramBot; from ductor_bot.transcription import transcribe_openai_audio; assert Path(ductor_bot.__file__).is_relative_to(Path('$wheel_site')); print('installed_wheel_import=PASS')"
