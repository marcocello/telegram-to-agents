...
- Producer/activation: create and resume a session, feed `/new`, send another turn, then load the same Telegram session under the other configured provider.
- Consumer: command handler and persisted session manager.
- Read-back: `/new` causes a new native ID; the provider switch starts a fresh provider-native session and persists its provider instead of sending an incompatible ID.
- Fake: Telegram Bot API and external provider processes only.
- Catches: cosmetic resets, cross-provider session reuse, stale IDs, or native configuration mutation.
...
- Producer/activation: run delayed provider streams that emit commentary/tool status and assistant answer data, plus streams that fail after accepting a turn or end without a terminal result.
- Consumer: CLI streaming service and Telegram delivery path.
- Read-back: status is visible before completion, answer deltas are not published as repeated edits, temporary progress deletion succeeds before exactly one clean final answer is sent, and each Telegram update produces exactly one provider turn request even on error. Forced progress-deletion failure withholds the final answer and records a transport-delivery error rather than posting a second message.
- Fake: Telegram Bot API and external provider processes only.
- Catches: vestigial pseudo-streaming, progress leaking into final output, duplicate final messages, and silent fallback replay.
...
- Consumer: generated config, runtime entrypoint, command registry, platform service selection, metadata, dependency graph, source tree, tests, docs, and wheel contents.
- Read-back: onboarding asks only for provider, Telegram authorization, project, and optional transcription; configuration accepts exactly Codex/Claude and has no prompt-mode choice; legacy agent settings are removed; only retained commands remain; Linux service support is present while macOS/Windows and automatic image conversion are absent; package dependencies and allowlisted contents match the focused product.
- Fake: terminal input, native harness auth probes, and Telegram's external API.
- Catches: making pass-through configurable, hiding dead code behind packaging exclusions, retaining alternate providers/transports/platform services, losing transcription, or leaving Pillow/YAML-only indexing dependencies behind.

## Scenario: Telegram security and topic projects remain gateway-owned

- Producer/activation: submit unauthorized and authorized updates, learn a forum-topic name, then exercise text, `/where`, media, and in/out-of-project file delivery before and after restart.
- Consumer: Telegram authorization middleware, topic cache, project resolver, provider start request, and allowed-root sender.
- Read-back: unauthorized input never reaches a harness; the mapped absolute project is used consistently and persists; in-project files send while out-of-project files are rejected.
- Fake: Telegram Bot API and external provider process only.
- Catches: treating pass-through as bypassing transport security, losing topic names, or resolving different roots across gateway paths.
...
- The source and installed wheel are lean and Linux-only.
