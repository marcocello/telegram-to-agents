...
- Producer/activation: create a representative `~/.ductor` tree containing configuration, provider sessions, `.env`, and Telegram files, then start the real renamed entrypoint with no new state directory; separately exercise new-and-legacy coexistence and an explicit new state root. Install the Linux service against fake outer systemd commands with a legacy unit present, then inject legacy stop and teardown failures. Run onboarding/config validation, build both wheel and sdist, install the wheel in an isolated target, and start its real entrypoint in a clean temporary environment.
