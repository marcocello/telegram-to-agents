...
- Telegram user/group/channel authorization, mention policy, file download/send policy, topic isolation, reply context, streaming limits, timeout handling, service lifecycle, restart handling, and logging remain transport responsibilities.
...
- The source checkout itself contains only the runtime, tests, assets, and active documentation needed by this gateway. Removed product implementations are deleted rather than retained behind packaging exclusions, and obsolete tests/default workspace assets are deleted with them.
