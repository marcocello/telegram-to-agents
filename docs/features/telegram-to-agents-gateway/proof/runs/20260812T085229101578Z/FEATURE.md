...
- Runtime, packaging, help output, service files, and active documentation contain no former Ductor branding or identifiers except the original MIT copyright/attribution, an explicit LICENSE notice that telegram-to-agents is a derived version of Ductor by PleasePrompto, and narrowly scoped legacy state/service migration constants and tests.
