...
- The renamed product is consistently distributed and executed as `telegram-to-agents`, imports only as `telegram_to_agents`, stores new state under `~/.telegram-to-agents`, replaces the legacy Linux service without a second poller, and preserves existing `~/.ductor` configuration, sessions, secrets, and files through bounded legacy migration.
...
## Scenario: Product identity is renamed, migrates legacy state, and remains lean

- Producer/activation: create a representative `~/.ductor` tree containing configuration, provider sessions, `.env`, and Telegram files, then start the real renamed runtime with no new state directory; separately activate an environment-selected root, a custom migrated legacy root, new-and-legacy coexistence, and an explicit new state root. Exercise runtime lock acquisition plus status and stop against the same selected root. Install the Linux service against fake outer systemd commands with a legacy unit present, inject legacy stop and teardown failures, then launch its real generated command under only the generated systemd environment and exercise configuration, PID, status, and stop. Run onboarding/config validation, build both wheel and sdist, install the wheel in an isolated target, and start its real entrypoint in a clean temporary environment.
- Consumer: state-root resolver and config migration, generated config, runtime entrypoint, command registry, Linux service installer, package metadata, dependency graph, source tree, tests, docs, GitHub templates/workflows, license, and wheel/sdist contents.
- Read-back: legacy-only state is copied completely to `~/.telegram-to-agents` without deleting the original, the legacy config key is rewritten to `state_home`, a custom legacy root is preserved and populated without competing runtime state, existing new state wins without merging, and an explicit environment root bypasses default migration. Runtime lock acquisition, status, and stop all consume the selected root rather than a default decoy. Successful service installation stops/disables/removes `ductor.service` before enabling only `telegram-to-agents.service`; every injected legacy teardown failure leaves the new unit unwritten, disabled, and unstarted. The wheel and sdist are both named for `telegram_to_agents-0.1.0`, contain `telegram_to_agents` and no `ductor_bot`, expose only the `telegram-to-agents` command, and the installed entrypoint reports the renamed product. Onboarding asks only for provider, Telegram authorization, project, and optional transcription; configuration accepts exactly Codex/Claude and has no prompt-mode or Codex-transport choice; legacy agent settings are removed; only retained commands remain; Linux service support is present while macOS/Windows service backends and automatic image conversion are absent. Active docs and package metadata use `https://github.com/marcocello/telegram-to-agents`; LICENSE preserves upstream MIT attribution and explicitly states that telegram-to-agents is a derived version of Ductor by PleasePrompto. Former identifiers are absent outside the explicit attribution/migration allowlist. README and installation docs state the automatic managed/embedded split, foreground macOS support, and Linux-only service deployment without claiming a GUI protocol.
- Fake: terminal input, native harness auth probes, and Telegram's external API.
- Catches: a cosmetic-only rename, broken imports or console scripts, parallel old/new Telegram pollers, lost legacy configuration/sessions/secrets/files, destructive legacy migration, precedence bugs when both roots exist, stale package/repository/service branding, missing MIT attribution, making pass-through configurable, hiding dead code behind packaging exclusions, retaining alternate providers/transports/platform services or obsolete contradictory feature packages, losing transcription, or leaving unused dependencies behind.
