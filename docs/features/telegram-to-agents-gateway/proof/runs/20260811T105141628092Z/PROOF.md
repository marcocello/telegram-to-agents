...
- Read-back: onboarding asks only for provider, Telegram authorization, project, and optional transcription; configuration accepts exactly Codex/Claude and has no prompt-mode choice; legacy agent settings are removed; only retained commands remain; Linux service support is present while macOS/Windows and automatic image conversion are absent; the issue template offers only the current Linux/Codex/Claude surface; package dependencies and allowlisted runtime, test, root, GitHub, and active-documentation contents match the focused product.
