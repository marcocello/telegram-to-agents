...
- The deployable background service targets Linux VMs only. Foreground source execution may use a local Codex installation on macOS through the same automatic transport selection, but Ductor has no macOS or Windows service backend.
