...
- A selected existing project directory is the default `cwd` for Codex threads. A valid per-topic project mapping may override it by topic name, `chat:topic`, or topic ID. Codex execution, `/where`, Telegram media handling, and workspace-scoped file delivery use the same resolved project for that topic. Invalid or missing project directories fail clearly instead of falling back to Ductor's shared workspace.
